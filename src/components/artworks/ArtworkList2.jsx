...
import { getAllArtworks, getAllGenres, getAllMediums } from "../../services/artworkService.js"
...
export const ArtworkList = ({ currentUser }) => {
    ...
    }, [showFavoritesOnly, selectedFilterOption])
    ...
    useEffect(() => {
        if (selectedFilterOption === 1) {
            ...
            if (showFavoritesOnly) {
                ...
                if (genreOption) {
                    const foundArtworks = allArtworks.filter(artwork => artwork.genreId === genreOption)
                    setFilteredArtworks(foundArtworks)
                }
        } else if (selectedFilterOption === 6) {
                if (mediumOption) {
                    const foundArtworks = allArtworks.filter(artwork => artwork.mediumId === mediumOption)
                    setFilteredArtworks(foundArtworks)
                }
